...
//////////////////////////////////////////////////////////////////////////////////////
// Memory handling
SEXP getResultsCrrEnet(double *a, double *resid, double *eta, double *st, double *w, double *diffBeta, double *accNum1, double *accNum2, double *accSum,
...
SEXP ccd_dense_enet(SEXP x_, SEXP t2_, SEXP ici_, SEXP wt_,
                    SEXP eps_, SEXP max_iter_, SEXP alpha_, SEXP lambda_, SEXP mult_) {
  ...
  for (int l = 0; l < L; l++) {
    ...
    while (INTEGER(iter)[l] < max_iter) {
      ...
      for (int j = 0; j < p; j++) {
        ...
        l1 = lam[l] * m[j];
        b[l * p + j] = getElasticNet(grad, hess, a[j], l1, alpha);
  ...
  res = getResultsCrrEnet(a, resid, eta, st, w, diffBeta, accNum1, accNum2, accSum,
