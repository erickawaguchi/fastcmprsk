...
//////////////////////////////////////////////////////////////////////////////////////
//Coordinate Descent Algorithm for sparse Fine-Gray Model

/*
CSR triple: row_ptr, col, x_ij
Data is sorted by covariate. So row_ptr is actually pointing to covariate columns and col represents observation #
*/

SEXP ccd_sparse_pen(SEXP x_, SEXP row_ptr_, SEXP col_idx_, SEXP ncov_, SEXP t2_, SEXP ici_, SEXP wt_,
                    SEXP eps_, SEXP max_iter_, SEXP penalty_, SEXP lambda_, SEXP mult_,
                    SEXP gamma_) {

  //Declaration
  int n = length(t2_);
  int p = INTEGER(ncov_)[0];
  ...
  double *x = REAL(x_);
  int *row_ptr = INTEGER(row_ptr_);
  int *col_idx = INTEGER(col_idx_);
  double *t2 = REAL(t2_);
  double *wt = REAL(wt_);
  ...
  for(int l = 0; l < L; l++) {
    ...
    while (INTEGER(iter)[l] < max_iter) {
      ...
      for (j = 0; j < p; j++) {
        grad = -getSparseWeightedCrossProduct(x, row_ptr, col_idx, w, r, j); // jth component of gradient [l'(b)]
        hess = getSparseWeightedSumSquares(x, row_ptr, col_idx, w, j); // jth component of hessian [l''(b)]
